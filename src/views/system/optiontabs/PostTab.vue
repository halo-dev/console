<template>
  <div>
    <a-form ref="postOptionsForm" :model="options" :rules="rules" layout="vertical" :wrapperCol="wrapperCol">
      ...
      <a-form-item label="首页每页文章条数：">
        <a-input-number v-model="options.post_index_page_size" :min="1" style="width:100%" />
      </a-form-item>
      <a-form-item label="归档页面每页文章条数：" help="* 包括标签所属文章页面、分类所属文章页面">
      ...
      <a-form-item label="自动清理回收站文章：">
        <a-switch v-model="options.recycled_post_cleaning_enabled" />
      </a-form-item>
      <a-form-item v-show="options.recycled_post_cleaning_enabled" label="回收站文章保留时长：">
